  ...
  size_t step, std::string varName, BlockSummary& dataSummary, void* dataContainerSrc)
{
  ...
  RawDataEndpoint rde(
    "", dataSummary.m_blockid, dataSummary.m_dims, dataSummary.m_indexlb, dataSummary.m_indexub);
  ...
  if (ifp.m_putMethod.compare(FILEPUT) == 0)
  {
    ...
    // update the datatype in raw data endpoint
    rde.m_dataType = DATATYPE_RAWFILE;
    strcpy(dataSummary.m_dataType, DATATYPE_RAWFILE.data());

    // write the data
    int status = b_manager.putBlock(dataSummary, BACKENDFILE, dataContainerSrc);
    if (status != 0)
    {
  ...
  else if (ifp.m_putMethod.compare(RDMAPUT) == 0)
  {
    ...
    rde.m_rawDataServerAddr = this->m_associatedDataServer;
  ...
  for (auto it = ifp.m_metaServerList.begin(); it != ifp.m_metaServerList.end(); it++)
  {
    ...
    int status = remotePutMetaData.on(metaserverEndpoint)(step, varName, rde);
    if (status != 0)
    {
...
std::vector<RawDataEndpoint> UniClient::getrawDataEndpointList(std::string serverAddr, size_t step,
  std::string varName, size_t dims, std::array<int, 3> indexlb, std::array<int, 3> indexub)
{
  tl::remote_procedure remotegetrawDataEndpointList =
    this->m_clientEnginePtr->define("getRawDataEndpointList");
  tl::endpoint serverEndpoint = this->lookup(serverAddr);
  std::vector<RawDataEndpoint> rawDataEndpointList =
    remotegetrawDataEndpointList.on(serverEndpoint)(step, varName, dims, indexlb, indexub);
  ...
  size_t elemSize, size_t dims, std::array<int, 3> indexlb, std::array<int, 3> indexub)
{
  ...
  for (auto it = metaList.begin(); it != metaList.end(); it++)
  {
    ...
    while (true)
    {
      ...
      // std::endl; std::cout << "metaServerAddr " << metaServerAddr << " size of rweList " <<
      // rweList.size()
      // << std::endl;
      if (rweList.size() == 0)
      {
    ...
    for (auto itrwe = rweList.begin(); itrwe != rweList.end(); itrwe++)
    {
      ...
      if (itrwe->m_dataType == DATATYPE_RAWMEM)
      {
        // get the subregion by the rdma to call the remote server
        int status = this->getSubregionData(itrwe->m_rawDataServerAddr, itrwe->m_rawDataID,
          allocSize, dims, itrwe->m_indexlb, itrwe->m_indexub, subDataContainer);
      ...
      else if (itrwe->m_dataType == DATATYPE_RAWFILE)
      {
        // get the subregion data from the local block management store
        BlockSummary bs = this->b_manager.getBlockSubregion(itrwe->m_rawDataID, BACKENDFILE, dims,
          itrwe->m_indexlb, itrwe->m_indexub, subDataContainer);
