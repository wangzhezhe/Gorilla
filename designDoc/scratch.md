...
### when the partitions of the simulation's view cover multiple partitions of the staging's view

After data put, we will caculated the overlappin between the bbx written into the server and the bbx that every staging server responsible for , then we will get multiple partitions. Then the client will update the server based on these partitions. It is possible that one partition from the data put is divided into the several smaller partitions when udpate the metadata. Every small partition belongs to one area from the staging's perspective.

Therefore, every raw data id may correspond to multiple data partitions, but for every metadata server, the rawid on that meta data server is supposed to be the unique.
...
### different data ouptut strategies in data staging by multi threads model

It is ok to output the data into the vtk format by multi tread model executed at the multiple data staging service.
There are some issues that use the ADIOS to output the data, the core reason is that ADIOS does not support the multithread model well. We tried to use some strategies to output the data into the ADIOS file, refer to [this issue](https://github.com/ornladios/ADIOS2/issues/2076), but it does not work properly.

### the status and the lifecycle of the data in staging service

Currently, there are following status for the rawdata for the convenince of the data management in memory:

**RAW**
this means the data is the original status, if the data is put into the server, it is in this status.

**BEFOREPROCESS**
this means the data is prepared to be processed by the trigger (in-memory functions), but the thread is not scheduled yet to process coresponding data.

**INPROCESS**
this means the data is started to be processed by the in-memory functions and the data is still in the process and not finished yet.

**AFTERPROCESS**
this means the data is finished processing and it is not interesting data.

**UNDELETABLE**
this means the data is finished processing and it is interesting data for the users.

the status of BEFOREPROCESS, INPROCESS, AFTERPROCESS and UNDELETEBLE can happen only when triggers are registered. When the system try to delete the out-of-date data, it will check the status label, only the labels that are RAW or the AFTERPROCESS can be deleted.

for the data blocks that are UNDELETEBLE and the INPROCESS or BEFOREPROCESS, they will be deleted by the consumer in a explicit way by calling RPC.

### the window maintained by the staging service

The memory space is limited, it is important to maintain the window of the server (the parameter `m_windowSize` in the metadataManager) and reuse the memory space. There are two ways to delete the data, one way is to delete the data by the data consumer, after finish the data read operaion, it can send the RPC to the data staging service to delete all the data generated by specific time step. Another is to let the staging server delete the old data automatically. For example, the window size is 10, the data generated at the first time step will be deleted when the simulation moves to the 11 step. But there are another issues, if there are some interesting properties of the data and it is not consumed by the reader, the data can not be deleted even if it is out of the buffer. the staging service delete the data according to the status and the lifecycle of the data object described above.

Besides, There should be a variable to count the actual total time steps stored at current server, before putting opertaion, if the current server is full, do not put the data, and hang there to wait for the free space (this is one of the future work, we consider add a monitor manager here to manage all monitored information for particular variable, the first level key here can be the variable name)

### the data structure of the metadata maintained by the matadata manager

the core component is the metadata maintained by the staging service. For the time based data
One step main contains multiple varaibles (the data of the varaible is array in this scenario)
one varaible contains multiple types, such as raw data or compressed data or datas in other format etc.
one type correspond to multiple data blocks
every data block corresponds to one endpoint that index the place where the raw data is stored.

Basically, the current metadata is a structure that contains multiple layer map.
But there are some issues here. that map is a little bit large and might be accessed by different thread. For example, the endpoint of the data structure for raw data contains the status of the data, we need to lock the whole data structure when there are updates of the data structure.
