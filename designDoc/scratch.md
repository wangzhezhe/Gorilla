...
There are several typical data managed by gorilla service. The `dataBlock`, it is just a chunk of memory space, typically, it stores the array generated by the simulation, but it can be other types of data such as the marshaled vtk data. The `blockSummary`, it might be more clear to call it block meta, it records key info of the block data, such as its format, its bounding box (which is common for grid simulation), how to store this blocks, such as in memory or by the file. Currently, we use the rawdataManager to do these things, but it might be more precise to call it the `blockDataManager`, try to update the name in future.

The next level is the index data, we use `MetaDataManager` to store these data, it might be more precise to call it `IndexDataManager`, we might update it in future. If we want to get the data with specific name in particular step and in which bounding box, we need these index information. Essentially, it is multi level search tree (we use multi level map currently). The leave node, namely the endpoint recorded by the index tree is called the `blockDescriptor`, it might be more precise to call it `blockIndexInformation`, which is the entry to access the actual object, the main information here is the block id, where it is stored (if the data is managed by the in-memory objects) and the indexed information such as bbx.


Based on these data model, there are quite flexibiilty to manage them. If we use the memory object driver, the `dataBlock` and  `blockSummary` should be managed manually by in-memory data objects. If there is not enough space for memory, the `dataBlock` and  `blockSummary` should be stored in the file. When the client want to get these data, after they get the index info such as `blockIndex` then they can extract the `blockSummary` and the `dataBlock` (maybe call it BlockData in future).


differnet dimentions to define a data block

backend: mem or file, also the mem pointer, which only keeps a pointer such as vtkDataSet instead of keeping the raw data compared with the rawmem or rawfile.

dataformat, cartisian grid, raw array, or vtk or others

block position, local or staging...

the endpoint information stored at the index (metadata server) need to know the both backend and the datatype of indexed data
