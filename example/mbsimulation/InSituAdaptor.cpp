...
void UpdateVTKAttributes(Mandelbulb& mandelbulb, int rank,
                         vtkCPInputDataDescription* idd) {
  vtkMultiPieceDataSet* multiPiece =
      vtkMultiPieceDataSet::SafeDownCast(VTKGrid->GetBlock(0));
  if (idd->IsFieldNeeded("mandelbulb", vtkDataObject::POINT)) {
    ...
    vtkIntArray* data = vtkIntArray::SafeDownCast(
        dataSet->GetPointData()->GetArray("mandelbulb"));
    // The pressure array is a scalar array so we can reuse
    // memory as long as we ordered the points properly.
    int* theData = mandelbulb.GetData();
    data->SetArray(
        theData, static_cast<vtkIdType>(mandelbulb.GetNumberOfLocalCells()), 1);
  }
}

void UpdateVTKAttributesList(std::vector<Mandelbulb>& mandelbulbList,
                             vtkCPInputDataDescription* idd) {
  ...
  if (idd->IsFieldNeeded("mandelbulb", vtkDataObject::POINT)) {
    for (int i = 0; i < pieceNum; i++) {
      ...
      data->SetArray(
          theData,
          static_cast<vtkIdType>(mandelbulbList[i].GetNumberOfLocalCells()), 1);
    }
  }
}

void BuildVTKDataStructures(Mandelbulb& mandelbulb, int nprocs, int rank,
                            vtkCPInputDataDescription* idd) {
  if (VTKGrid == NULL) {
...
void BuildVTKDataStructuresList(std::vector<Mandelbulb>& mandelbulbList,
                                int global_nblocks,
                                vtkCPInputDataDescription* idd) {
...
void MPIInitialize(const std::string& script) { return; }

void MochiInitialize(const std::string& script) { return; }

void Finalize() { return; }

void MPICoProcessDynamic(MPI_Comm subcomm,
                         std::vector<Mandelbulb>& mandelbulbList,
                         int global_nblocks, double time,
...
void MPICoProcess(Mandelbulb& mandelbulb, int nprocs, int rank, double time,
                  unsigned int timeStep) {
  return;
}

void MochiCoProcess(Mandelbulb& mandelbulb, int nprocs, int rank, double time,
                    unsigned int timeStep) {
  return;
}

void StoreBlocks(unsigned int timeStep,
                 std::vector<Mandelbulb>& mandelbulbList) {
...
void writeVTKData(std::vector<Mandelbulb>& mandelbulbList, int global_blocks,
                  std::string fileName) {
  int i = 0;
  for (auto& mandelbulb : mandelbulbList) {
    int* extents = mandelbulb.GetExtents();
...
void sampleExtraction(std::vector<Mandelbulb>& mandelbulbList,
                      int global_blocks) {
  // sample the data
  int i = 0;
  for (auto& mandelbulb : mandelbulbList) {
...
void sampleisoExtraction(std::vector<Mandelbulb>& mandelbulbList,
                         int global_blocks) {
  // sample the data
  int i = 0;
  for (auto& mandelbulb : mandelbulbList) {
...
int putData(void* dataSourcePtr, size_t datasize, std::string dir,
            std::string filename) {
...
void StoreBlocksSharedMem(unsigned int timeStep,
                          std::vector<Mandelbulb>& mandelbulbList) {
...
void StoreBlockDisk(std::vector<Mandelbulb>& mandelbulbList, std::string dir,
                    std::string filename) {
