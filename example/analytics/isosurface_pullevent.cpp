...
void writeImageData(
  std::string fileName, std::array<int, 3>& indexlb, std::array<int, 3>& indexub, void* fielddata)
{
  auto importer = vtkSmartPointer<vtkImageImport>::New();
  importer->SetDataSpacing(1, 1, 1);
  importer->SetDataOrigin(0, 0, 0);
  // from 0 to the shape -1 or from lb to the ub??
  importer->SetWholeExtent(indexlb[2], indexub[2], indexlb[1], indexub[1], indexlb[0], indexub[0]);
...
vtkSmartPointer<vtkPolyData> compute_isosurface(std::array<size_t, 3>& shape,
  std::array<size_t, 3>& offset, const std::vector<double>& field, double isovalue)
{
  // Convert field values to vtkImageData
  auto importer = vtkSmartPointer<vtkImageImport>::New();
  importer->SetDataSpacing(1, 1, 1);
  importer->SetDataOrigin(1.0 * offset[2], 1.0 * offset[1], 1.0 * offset[0]);
  importer->SetWholeExtent(0, shape[2], 0, shape[1], 0, shape[0]);
  importer->SetDataExtentToWholeExtent();
  importer->SetDataScalarTypeToDouble();
  importer->SetNumberOfScalarComponents(1);
...
void writePolyvtk(const std::string& fname, const vtkSmartPointer<vtkPolyData> polyData)
{
...
DynamicTriggerInfo parseTrigger(nlohmann::json& j)
{
  ...
  DynamicTriggerInfo tgInfo(funcNameCheck, ParametersCheck, funcNameCompare, ParametersCompare,
    funcNameAction, ParametersAction);

  return tgInfo;
}

std::string loadTrigger(ClientForSim* m_uniclient, std::string fileName)
{
...
int main(int argc, char* argv[])
{
  ...
  ClientForSim* uniclient = new ClientForSim(&clientEngine, addrServer, rank);
  ...
  while (true)
  {
    ...
    while (true)
    {
      if (rank == 0)
      {
        ...
        // event.printInfo();
        step = event.m_step;
        // if the event does not follow the sequence such as 1 1 1 6 6 6 1 1 1, the data for step 1
        // might be pulled for another time
      ...
      if (rank < 5)
      {
        std::cout << "sample, rank " << rank << " pull variable " << VarNameU << " for step "
                  << step << std::endl;
      }
  ...
  */
      ...
      std::array<int, 3> indexub = { { offset_x + size_x - 1, offset_y + size_y - 1,
        offset_z + size_z - 1 } };
      ...
      MATRIXTOOL::MatrixView dataView =
        uniclient->getArbitraryData(step, VarNameU, sizeof(double), 3, indexlb, indexub);
      ...
      if (rank == 0)
      {
        ...
        uniclient->endTimer(uniclient->m_addrServer);
        // delete old data
    ...
    // auto polyData = compute_isosurface(blockmeta.m_shape, blockmeta.m_offset, dataContainer,
    // isovalue);
