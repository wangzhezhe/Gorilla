...
void AmrCoreAdv::MakeNewLevelFromCoarse(
...
// overrides the pure virtual function in AmrCore
void AmrCoreAdv::RemakeLevel(int lev, Real time, const BoxArray& ba, const DistributionMapping& dm)
{
...
// overrides the pure virtual function in AmrCore
void AmrCoreAdv::MakeNewLevelFromScratch(
  int lev, Real time, const BoxArray& ba, const DistributionMapping& dm)
{
  const int ncomp = 1;
  const int nghost = 0;
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
  ...
  for (MFIter mfi(state, TilingIfNotGPU()); mfi.isValid(); ++mfi)
  {
    ...
    amrex::launch(box, [=] AMREX_GPU_DEVICE(Box const& tbx) { initdata(tbx, fab, problo, dx); });
  }
}

// tag all cells for refinement
// overrides the pure virtual function in AmrCore
void AmrCoreAdv::ErrorEst(int lev, TagBoxArray& tags, Real /*time*/, int /*ngrow*/)
{
...
void AmrCoreAdv::AverageDown()
{
  ...
  {
    amrex::average_down(phi_new[lev + 1], phi_new[lev], geom[lev + 1], geom[lev], 0,
...
{
  amrex::average_down(phi_new[crse_lev + 1], phi_new[crse_lev], geom[crse_lev + 1], geom[crse_lev],
...
void AmrCoreAdv::FillPatch(int lev, Real time, MultiFab& mf, int icomp, int ncomp)
{
  ...
  else
  {
    ...
    if (Gpu::inLaunchRegion())
    {
      ...
      amrex::FillPatchTwoLevels(mf, time, cmf, ctime, fmf, ftime, 0, icomp, ncomp, geom[lev - 1],
...
void AmrCoreAdv::FillCoarsePatch(int lev, Real time, MultiFab& mf, int icomp, int ncomp)
{
  BL_ASSERT(lev > 0);
  ...
  if (Gpu::inLaunchRegion())
  {
    ...
    amrex::InterpFromCoarseLevel(mf, time, *cmf[0], 0, icomp, ncomp, geom[lev - 1], geom[lev],
      cphysbc, 0, fphysbc, 0, refRatio(lev - 1), mapper, bcs, 0);
  }
  else
  {
    CpuBndryFuncFab bndry_func(nullptr); // Without EXT_DIR, we can pass a nullptr.
...
void AmrCoreAdv::timeStepWithSubcycling(int lev, Real time, int iteration)
{
...
void AmrCoreAdv::WritePlotFile() const
{
  ...
  amrex::WriteMultiLevelPlotfile(
    plotfilename, finest_level + 1, mf, varnames, Geom(), t_new[0], istep, refRatio());
}
...
void AmrCoreAdv::WriteCheckpointFile() const
{
  ...
  // chk00010/Header     this contains information you need to save (e.g., finest_level, t_new,
  // etc.) and also
  ...
  if (ParallelDescriptor::IOProcessor())
  {
    ...
    HeaderFile.open(
      HeaderFileName.c_str(), std::ofstream::out | std::ofstream::trunc | std::ofstream::binary);
    if (!HeaderFile.good())
    {
