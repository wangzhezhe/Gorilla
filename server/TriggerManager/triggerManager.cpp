...
void DynamicTriggerManager::initstart(std::string triggerName, size_t step, std::string varName, BlockDescriptor rde)
{
    ...
    this->m_metadataManager->updateMetaDataStatus(step, varName, DATATYPE_CARGRID, rde.m_rawDataID, MetaStatus::INPROCESS);
    ...
    //execute checking
    //size_t step, std::string varName, BlockDescriptor& rde
    ...
    int status = this->m_metadataManager->getMetaDataStatus(step, varName, DATATYPE_CARGRID, rde.m_rawDataID);
    ...
    {
        this->m_metadataManager->updateMetaDataStatus(step, varName, DATATYPE_CARGRID, rde.m_rawDataID, MetaStatus::AFTERPROCESS);
    }
...
//common start
void DynamicTriggerManager::commonstart(std::string triggerName, size_t step, std::string varName, BlockDescriptor rde)
{
    if (this->m_dynamicTrigger.find(triggerName) == this->m_dynamicTrigger.end())
    {
        throw std::runtime_error("failed to get the trigger with name " + triggerName);
